...
ENGLISH_LANGUAGE_MARKS = {"eng", "en", "english"}
...
def probe_subtitle_stream(ffprobe: str, media_path: str) -> int | None:
    ...
    def rank(stream: dict[str, object]) -> tuple[int, int]:
        tags = stream.get("tags", {})
        if not isinstance(tags, dict):
            tags = {}
        language = str(tags.get("language", "")).lower()
        title = str(tags.get("title", "")).lower()
        is_english = language in ENGLISH_LANGUAGE_MARKS or any(mark in title for mark in ENGLISH_LANGUAGE_MARKS)
        return (0 if is_english else 1, int(stream.get("index", 9999)))
...
def extract_embedded_subtitle(media_path: str) -> tuple[str | None, str | None]:
    ...
    english_cached = subtitle_cache_path(media_path, "english")
    commands.append([ffmpeg, "-y", "-i", media_path, "-map", "0:s:m:language:eng?", "-c:s", "srt", english_cached])
    commands.append([ffmpeg, "-y", "-i", media_path, "-map", "0:s:m:language:en?", "-c:s", "srt", english_cached])
