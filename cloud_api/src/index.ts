...
const DESKTOP_LATEST = {
  ...
  url: 'https://github.com/mjandreas125/subtitle-notes/releases/latest/download/SubtitleNotesSetup-1.6.2.exe',
  notes: '',
};
...
const libraryPage = (lang: string) => `<!doctype html>
<html lang="${lang}"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Subtitle Notes — library</title>
<script src="https://accounts.google.com/gsi/client" async></script>
<style>
  :root { --paper:#faf8f4; --ink:#14201c; --soft:#6b7a74; --hair:#e2ddd3; --accent:#1e7a4c; --wash:#e7f2ea; --red:#b4463c; }
  @media (prefers-color-scheme: dark) { :root { --paper:#151b19; --ink:#eaf1ed; --soft:#93a49c; --hair:#2a3733; --accent:#64c795; --wash:#1d2a25; --red:#f08078; } }
  * { box-sizing:border-box } body { margin:0; min-height:100dvh; background:var(--paper); color:var(--ink); font:15px/1.45 ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,sans-serif; }
  header { position:sticky; top:0; z-index:2; display:flex; align-items:center; gap:14px; padding:14px max(20px,calc((100vw - 1100px)/2)); border-bottom:1px solid var(--hair); background:color-mix(in srgb,var(--paper) 94%,transparent); backdrop-filter:blur(12px); }
  .mark { display:grid; place-items:center; width:34px; height:34px; border-radius:10px; background:var(--accent); color:#fff; font-size:18px } h1 { margin:0; font-size:19px; letter-spacing:-.02em } .count { color:var(--soft); font-size:13px; font-weight:650 } .spacer { flex:1 }
  button, input { font:inherit } button { border:0; border-radius:9px; padding:9px 12px; color:white; background:var(--accent); font-weight:700; cursor:pointer } button:hover { filter:brightness(1.06) } button.ghost { color:var(--soft); background:transparent } button.ghost:hover { color:var(--ink); background:var(--wash) } button.danger { background:transparent; color:var(--red) }
  main { width:min(1100px,100%); margin:0 auto; padding:28px 20px 44px } .toolbar { display:flex; gap:10px; align-items:center; margin-bottom:22px } .toolbar input { flex:1; min-width:0; padding:10px 12px; border:1px solid var(--hair); border-radius:9px; color:var(--ink); background:transparent; outline-color:var(--accent) }
  .due { display:none; margin-bottom:22px; padding:18px; border:1px solid color-mix(in srgb,var(--accent) 35%,var(--hair)); border-radius:14px; background:var(--wash) }.due.show { display:block }.due h2 { margin:0 0 6px; font-size:16px }.due p { margin:0;color:var(--soft) }.due button { margin:13px 7px 0 0 }.due .answer { display:none; margin-top:12px; font-size:19px; font-weight:750 }.due.reveal .answer { display:block }
  .grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:13px }.card { min-height:147px; padding:17px; border:1px solid var(--hair); border-radius:13px; background:color-mix(in srgb,var(--paper) 88%,white); box-shadow:0 1px 1px rgb(0 0 0/.03); cursor:pointer }.card:hover { border-color:color-mix(in srgb,var(--accent) 55%,var(--hair)); background:var(--wash) }.word { font-size:20px; font-weight:760; letter-spacing:-.02em; overflow-wrap:anywhere }.meaning { margin-top:6px; color:var(--accent); font-weight:650; overflow-wrap:anywhere }.source { margin-top:18px; color:var(--soft); font-size:12px; font-weight:650; white-space:nowrap; overflow:hidden; text-overflow:ellipsis }.empty { margin:80px auto; max-width:420px; text-align:center; color:var(--soft) }.empty h2 { color:var(--ink); font-size:20px }.detail { position:fixed; inset:0; z-index:4; display:none; place-items:center; padding:20px; background:rgb(0 0 0/.34) }.detail.show { display:grid }.sheet { width:min(560px,100%); max-height:85dvh; overflow:auto; padding:24px; border-radius:16px; background:var(--paper); box-shadow:0 22px 60px rgb(0 0 0/.25) }.sheet h2 { margin:0;font-size:26px;overflow-wrap:anywhere }.sheet .big { margin:7px 0 18px;color:var(--accent);font-size:18px;font-weight:700 }.sheet section { margin-top:15px;padding-top:15px;border-top:1px solid var(--hair) }.sheet label { display:block;margin-bottom:5px;color:var(--soft);font-size:12px;font-weight:750;text-transform:uppercase;letter-spacing:.06em }.sheet p { margin:0;white-space:pre-wrap;overflow-wrap:anywhere }.sheet .close { float:right;margin:-8px -8px 0 12px }
  #login { max-width:400px; margin:13dvh auto; text-align:center } #login h2 { font-size:28px; letter-spacing:-.03em } #login p { color:var(--soft) } #google { display:flex;justify-content:center;margin-top:22px } [hidden] { display:none!important }
</style></head><body>
<header><div class="mark">▤</div><div><h1>Subtitle Notes</h1><div id="count" class="count">Your library</div></div><div class="spacer"></div><button id="refresh" class="ghost" hidden>Refresh</button><button id="signout" class="ghost" hidden>Sign out</button></header>
<main><section id="login"><h2>Your words, on any computer.</h2><p>Sign in with the same Google account as on your phone. Nothing is installed and this browser only sees your own library.</p><div id="google"></div><p id="login-note"></p></section><section id="app" hidden><div id="due" class="due"><h2 id="due-title">Ready to practise</h2><p id="due-prompt"></p><div class="answer" id="due-answer"></div><div id="due-actions"><button id="reveal">Show meaning</button></div></div><div class="toolbar"><input id="search" type="search" autocomplete="off" placeholder="Search your words"><button id="clear" class="ghost">Clear</button></div><div id="grid" class="grid"></div><div id="empty" class="empty" hidden><h2>Nothing saved here yet</h2><p>Select a subtitle in VLC or in your browser and it will appear here.</p></div></section></main>
<div id="detail" class="detail"><article class="sheet"><button id="close" class="close ghost" aria-label="Close">✕</button><h2 id="detail-word"></h2><div id="detail-meaning" class="big"></div><section id="detail-context" hidden><label>Selected subtitle</label><p></p></section><section id="detail-translation" hidden><label>Full translation</label><p></p></section><section id="detail-variants" hidden><label>Other meanings</label><p></p></section><section id="detail-examples" hidden><label>Examples</label><p></p></section></article></div>
<script>
  const API = '/v1', TOKEN = 'subtitle-notes/library-token'; let cards = [], review = [];
  const $ = (id) => document.getElementById(id);
  const esc = (v) => String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const request = async (path, method = 'GET', body) => { const token = localStorage.getItem(TOKEN); const r = await fetch(API + path, { method, headers: { 'content-type':'application/json', ...(token ? {authorization:'Bearer ' + token} : {}) }, body: body ? JSON.stringify(body) : undefined }); const data = await r.json().catch(() => ({})); if (!r.ok) throw new Error(data.detail || 'Could not reach the library'); return data; };
  function renderCards() { const q = $('search').value.trim().toLowerCase(); const shown = cards.filter(c => !q || [c.focus_phrase,c.focus_word,c.selected_text,c.focus_translation,c.translation,c.media_title].some(v => String(v || '').toLowerCase().includes(q))); $('count').textContent = cards.length + (cards.length === 1 ? ' word' : ' words'); $('grid').innerHTML = shown.map(c => '<article class="card" data-id="' + esc(c.id) + '"><div class="word">' + esc(c.focus_phrase || c.focus_word || c.selected_text) + '</div><div class="meaning">' + esc(c.focus_translation || c.translation) + '</div><div class="source">' + esc(c.media_title || 'Subtitle') + '</div></article>').join(''); $('empty').hidden = shown.length !== 0; document.querySelectorAll('.card').forEach(n => n.onclick = () => openCard(n.dataset.id)); }
  async function openCard(id) { const item = await request('/selections/' + encodeURIComponent(id)); $('detail-word').textContent = item.focus_phrase || item.focus_word || item.selected_text; $('detail-meaning').textContent = item.focus_translation || item.translation; const sections = [['detail-context','Selected subtitle',item.selected_text],['detail-translation','Full translation',item.translation],['detail-variants','Other meanings',(item.variants || []).join('\\n')],['detail-examples','Examples',(item.examples || []).map(x => x.text + (x.translation ? '\\n' + x.translation : '')).join('\\n\\n')]]; sections.forEach(([id,label,value]) => { const node=$(id); node.hidden=!value; if(value){node.querySelector('label').textContent=label;node.querySelector('p').textContent=value;} }); $('detail').classList.add('show'); }
  function renderReview() { const item = review[0]; const box = $('due'); if(!item){box.className='due';return;} box.className='due'; $('due-title').textContent = review.length === 1 ? 'One word ready to practise' : review.length + ' words ready to practise'; $('due-prompt').textContent = item.focus_phrase || item.focus_word || item.selected_text; $('due-answer').textContent = item.focus_translation || item.translation; $('due-actions').innerHTML='<button id="reveal">Show meaning</button>'; $('reveal').onclick=()=>{box.classList.add('reveal');$('due-actions').innerHTML='<button data-result="again">Again</button><button data-result="good">I knew it</button><button data-result="easy">Easy</button>'; document.querySelectorAll('[data-result]').forEach(b=>b.onclick=()=>markReview(item.id,b.dataset.result));}; box.classList.add('show'); }
  async function markReview(id,result){ await request('/selections/'+encodeURIComponent(id)+'/review','POST',{result}); review.shift(); renderReview(); }
  async function load(){ try { [cards,review]=await Promise.all([request('/selections'),request('/review')]); $('login').hidden=true;$('app').hidden=false;$('refresh').hidden=false;$('signout').hidden=false;renderCards();renderReview(); } catch(e) { localStorage.removeItem(TOKEN); $('login').hidden=false;$('app').hidden=true;$('login-note').textContent=e.message; } }
  async function signedIn(response){ $('login-note').textContent='Opening your library…'; try { const data=await request('/auth/google','POST',{id_token:response.credential}); localStorage.setItem(TOKEN,data.token); await load(); } catch(e){$('login-note').textContent=e.message;} }
  window.addEventListener('load',()=>{ $('search').oninput=renderCards; $('clear').onclick=()=>{$('search').value='';renderCards();}; $('refresh').onclick=load; $('signout').onclick=()=>{localStorage.removeItem(TOKEN);load();}; $('close').onclick=()=>$('detail').classList.remove('show'); $('detail').onclick=e=>{if(e.target===$('detail'))$('detail').classList.remove('show');}; if(localStorage.getItem(TOKEN)){load();return;} const start=()=>{ if(!window.google?.accounts?.id){$('login-note').textContent='Google sign-in could not load. Check your connection and reload.';return;} google.accounts.id.initialize({client_id:${JSON.stringify(CLIENT_ID)},ux_mode:'popup',callback:signedIn}); google.accounts.id.renderButton($('google'),{theme:'filled_black',size:'large',shape:'pill',text:'continue_with',width:300});}; setTimeout(start,250); });
</script></body></html>`;
...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (path === '/library' || path === '/web-library') {
      return new Response(libraryPage(linkLanguage(url, request)), {
